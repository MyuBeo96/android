...
public class CustomChartView extends View implements ChartChangeListener,
	...
	public static final int DEFAULT_ZOOM_TRIGGER_DISTANCE = 0;
		...
		case MotionEvent.ACTION_POINTER_UP:
			...
			if (count == 1) {
				...
				if (mNowTimeMillis - mPrevTimeMillis < 400) {
					if (afchart.getPlot() instanceof Movable) {
						restoreAutoBounds();
						mScale = 1.0f;
						inertialMovedFlag = false;
					}
	...
	public void restoreAutoBounds() {
		Plot plot = this.afchart.getPlot();
		if (plot == null) {
			return;
		}
		// here we tweak the notify flag on the plot so that only
	...
	public void setChart(AFreeChart chart) {
		...
		if (this.afchart != null) {
			this.afchart.removeChangeListener(this);
			this.afchart.removeProgressListener(this);
		}

		// add the new chart
		this.afchart = chart;
		if (chart != null) {
			this.afchart.addChangeListener(this);
			this.afchart.addProgressListener(this);
	...
	public void paintComponent(Canvas canvas) {
		...
		// }
		this.afchart.draw(canvas, chartArea, this.anchor, this.info);
	...
	public void touched(MotionEvent event) {
		...
		this.afchart.setNotify(true); // force a redraw

		afchart.handleClick((int) event.getX(), (int) event.getY(), info);
		inertialMovedFlag = false;
